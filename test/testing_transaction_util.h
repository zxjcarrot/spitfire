...
struct TestTuple : spitfire::BaseTuple {
    uint64_t key;
    uint64_t value;
...
typedef DataTable<uint64_t, TestTuple> TestDataTable;
...
public:
    ...
    // when created
    static TestDataTable * CreateTable(
            int num_key = 10, ConcurrentBufferManager * buf_mgr = nullptr);

    static bool ExecuteInsert(TransactionContext *txn,
                              TestDataTable * table, int id, int value, ConcurrentBufferManager * buf_mgr);
    static bool ExecuteRead(TransactionContext *txn,
                            TestDataTable * table, int id, int &result,
                            bool select_for_update = false, ConcurrentBufferManager * buf_mgr = nullptr);
    ...
    static bool ExecuteScan(TransactionContext *txn,
                            ...
                            int id, bool select_for_update = false, ConcurrentBufferManager * buf_mgr = nullptr);
};
...
public:
    TransactionThread(TransactionSchedule *sched, TestDataTable *table_,
                      MVTOTransactionManager *txn_manager_)
            : schedule(sched),
    ...
    void ExecuteNext() {
        ...
        switch (op) {
            case TXN_OP_INSERT: {
                ...
                execute_result =
                        TestingTransactionUtil::ExecuteInsert(txn, table, id, value, txn_manager->GetBufferManager());
                break;
            }
            case TXN_OP_READ: {
                ...
                execute_result = TestingTransactionUtil::ExecuteRead(
                        txn, table, id, result, is_for_update, txn_manager->GetBufferManager());
            ...
            case TXN_OP_DELETE: {
                ...
                execute_result = TestingTransactionUtil::ExecuteDelete(txn, table, id,
                                                                       is_for_update, txn_manager->GetBufferManager());
                break;
            }
            case TXN_OP_UPDATE: {
                execute_result = TestingTransactionUtil::ExecuteUpdate(
                        txn, table, id, value, is_for_update, txn_manager->GetBufferManager());
            ...
            case TXN_OP_SCAN: {
                ...
                execute_result = TestingTransactionUtil::ExecuteScan(
                        txn, schedule->results, table, id, is_for_update, txn_manager->GetBufferManager());
                break;
            }
            case TXN_OP_UPDATE_BY_VALUE: {
                ...
                execute_result = TestingTransactionUtil::ExecuteUpdateByValue(
                        txn, table, old_value, new_value, is_for_update, txn_manager->GetBufferManager());
                break;
            }
            case TXN_OP_ABORT: {
    ...
    MVTOTransactionManager *txn_manager;
    TestDataTable *table;
    int cur_seq;
    bool go;
...
public:
    ...
    void Run() {
        ...
        if (!concurrent) {
            ...
            CountDownLatch latch(schedules.size());
            for (int i = 0; i < (int)schedules.size(); i++) {
